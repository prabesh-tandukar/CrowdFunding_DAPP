...
function App() {
  ...
  function setupEventListeners() {
    if (contract) {
      contract.on(
        "CampaignCreated",
        (campaignId, owner, title, target, deadline, category) => {
      ...
      contract.on("Withdrawal", (campaignId, owner, amount) => {
        console.log("Funds withdrawn:", { campaignId, owner, amount });
        fetchCampaigns();
      });
          ...
          {filteredCampaigns.map((campaign) => (
            <div key={campaign.id} className="border rounded p-4">
              ...
              <button
                ...
                disabled={campaign.ended || campaign.fundsWithdrawn}
              >
                Donate
              </button>
              <button
